//main.cpp
//A. Achimescu
// This program will read a bitmap image and output it's header information onto the program window but have the colour
//information be intputed into a text file.
...
#include "bitmap_variables.hpp"  //where the variables are
...
{
    bool numeric = true; //make sure numberStr is valid
    ...
    for (int u = 0; u <= size; u++)
    {
        ...
        if (!isdigit(numberStr[u]))
        {
            numeric = false;
            return numeric;
        }
        else //if number is valid, return a true value
            return numeric;
    }
}
...
bool answerYN ()
{
    ...
    bool type;      //what the program will give back
...
void chooseEffect(char* filename)
{
    ...
    while (goodbye)
    {
        ...
        if (answer)
        {
            cout << "Yay! Please choose which effect you would like to use on your image." << endl;
...
void resizeBMP (char* filename)
{
    ...
    while (!(width > 0))
    {
        cout << "Width: ";
        cin >> widthStr;
        if (isNumerical(widthStr)) //if a number, convert string into an int
        {
            stringstream ss(widthStr);
            ss >> width;
        }
        if (!isNumerical(widthStr) || width == 0) //if not a number, ask for the number again
        {
    ...
    while (!(height > 0))
    {
        cout << "Height: ";
        cin >> heightStr;
        if (isNumerical(heightStr)) //if a number, convert string into an int
        {
            stringstream ss(heightStr);
            ss >> height;
        }
        if (!isNumerical(heightStr) || height == 0) //if not a number, ask for the number again
        {
            cout << "Please enter a positive number" << endl;
            continue;
        }
...
void blurBMP(char* filename)
{
    ...
    {
        cin >> blurryStr;
        if (isNumerical(blurryStr)) //if a number, convert string into an int
        {
            stringstream ss(blurryStr);
            ss >> blurry;
        }
        if (!isNumerical(blurryStr)) //if not a number, ask for the number again
        {
            cout << "Please enter a positive number" << endl;
            continue;
        }
